...
public class PublishAckMessageStrategy implements MessageStrategy {
    ...
    public void sendResponseMessage(ChannelHandlerContext channelHandlerContext, MqttMessage mqttMessage) {
        ...
        log.info("===PublishAckMessageStrategy receive publish data:" + data);

        MqMessage mqMessage = new MqMessage();
        mqMessage.data = data;
        channelHandlerContext.fireChannelRead(mqMessage);
        ...
        switch (mqttQoS) {
            ...
            case AT_LEAST_ONCE:
                //至少一次
                processAtLeastOnceMsg(channelHandlerContext.channel(), mqttFixedHeader, packageId);
                break;
            case EXACTLY_ONCE:
                //刚好一次
                processExactlytOnceMsg(channelHandlerContext.channel(), mqttFixedHeader, packageId);
                break;
            default:
                break;
        }
    ...
    public void processAtLeastOnceMsg(Channel channel, MqttFixedHeader mqttFixedHeader, int packageId) {
        ...
        log.info("===PublishAckMessageStrategy response message:" + pubAckMsg.toString());
    ...
    public void processExactlytOnceMsg(Channel channel, MqttFixedHeader mqttFixedHeader, int packageId) {
        ...
        log.info("===PublishAckMessageStrategy 返回消息:" + pubAckMsg.toString());
